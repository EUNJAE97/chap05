...
public class Application2 {
    public static void main(String[] args) {
        ...
        // 참조형 자료형(reference type)이라고 한다.
        // 기본 자료형 : 실질적인 값
        // 참조 자료형 : 값의 주소
//        iarr = new int[5]; // 할당할 크기는 반드시 지정 되어야한다.
        ...
        *  stack에 저장 된 주소로 heap에 할당 된 배열을 찾아갈 수 있다. */
        ...
        //결과는 16진수로 표기된 주소값이 출력된다.

        /* hasCode(): 객체의 주소 값을 10진수로 변환하여 생성한 객체의 고유한 정수 값을 반환 하는 메소드*/
        System.out.println("iarr.hashCode() : " + iarr.hashCode());
        System.out.println("darr.hashCode() : " + darr.hashCode());

        /* 배열의 길이를 알 수 있는 기능을 필드로 제공한다.
        *  String 클래스의 문자열이 길이는 메소드로 제공하므로 length() 로 사용하는 점에 유의한다. */
        ...
        *  단, 참조되지 않은 배열은 일정 시간이 지난 후 heap의 old 영역으로 이동하여 GC(가비지 컬렉터)
        *  가 삭제 시킨다.
        *  한 번 찾아 갈 수 있는 주소 값을 잃어버린 배열은 다시 참조 불가능하다. */

        arr = null; /* null; 참조하는 주소 값이 없음을 나타냄 */

        /*java.lang.NullPointerException발생
